...
function createLocalesSettings(localesList) {
  // First, remove duplicates while preserving order
  const uniqueLocales = [...new Set(localesList)];
  
  return uniqueLocales.reduce((acc, locale, index) => {
    acc[locale] = {
      id: index,
      enabled: index === 0
    };
